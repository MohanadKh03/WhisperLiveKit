...
class WhisperLiveKit:
    ...
    def warmup_xtts(self):
        """
        Warmup the XTTS model by generating a short audio sample.
        """
        if not hasattr(self, 'xtts_model'):
            print("XTTS model is not initialized.")
            return
        
        # Generate a short audio sample
        text = "Hello, this is a warmup for the XTTS model."
        ...
        torchaudio.save("xtts_warmup.wav", torch.tensor(out["wav"]).unsqueeze(0), 24000)
        print("XTTS model warmed up and saved to xtts_warmup.wav")
